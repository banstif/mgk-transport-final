...
export function PaiementForm({
  ...
  const { data: factureResponse } = useFacture(factureId);
  ...
  }, [open, factureId, remainingAmount, reset]);
      ...
      <DialogContent className="sm:max-w-[450px]">
        ...
        <div className="rounded-lg border p-4 bg-muted/30 space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-medium">{formatCurrency(facture?.montantTTC || 0)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span>Déjà payé:</span>
