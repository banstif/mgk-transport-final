...
// POST /api/alertes/check-documents - Check all alerts (documents, factures, entretiens, contrats CDD)
export async function POST(
  ...
  try {
    ...
    const parts = [];
    if (results.documents > 0) parts.push(`${results.documents} document(s)`);
